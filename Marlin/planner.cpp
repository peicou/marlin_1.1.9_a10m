...
int16_t Planner::flow_percentage[E_STEPPERS] = ARRAY_BY_E_STEPPERS1(100); // Extrusion factor for each extruder

float Planner::e_factor[E_STEPPERS] = ARRAY_BY_E_STEPPERS1(1.0f); // The flow percentage and volumetric multiplier combine to scale E movement
...
void Planner::synchronize() { while (has_blocks_queued() || cleaning_buffer_counter) idle(); }
  ...
  void gradient_change(const int8_t start_p, const int8_t end_p, const float start_z, const float end_z) {
    if (WITHIN(current_position[Z_AXIS], start_z, end_z)) {
      ...
      mixing_factor[NOZZLE0] = mixer.rate[NOZZLE0] * 0.01;
      mixing_factor[NOZZLE1] = mixer.rate[NOZZLE1] * 0.01;
    }
