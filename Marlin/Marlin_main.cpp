...
bool filament_runout_enabled = false;
  ...
  float filament_change_unload_length[E_STEPPERS],
        filament_change_load_length[E_STEPPERS];
#endif
          ...
          for (int8_t PR_INNER_VAR = inStart; PR_INNER_VAR != inStop; PR_INNER_VAR += inInc) {
            ...
            yProbe = FLOOR(yBase + (yBase < 0 ? 0 : 0.5));
              ...
              if (!position_is_reachable_by_probe(xProbe, yProbe)) continue;
               ...
               // Arif: hack to compensate probe error causing right side lower than left
               measured_z = faux ? 0.001 * random(-100, 101) : probe_pt(xProbe, yProbe, raise_after, verbose_level)
                                  - X_TILT_COMP * ((xProbe / X_BED_SIZE) - 0.5)
                                  - Y_TILT_COMP * ((yProbe / Y_BED_SIZE) - 0.5);
             #else
               measured_z = faux ? 0.001 * random(-100, 101) : probe_pt(xProbe, yProbe, raise_after, verbose_level);
             #endif
...
inline void gcode_M17() {
  ...
  static bool load_filament(const float &slow_load_length=0, const float &fast_load_length=0, const float &purge_length=0, const int8_t max_beep_count=0,
    ...
    if (fast_load_length) {
      ...
      planner.e_factor[active_extruder] = 1; //Arif
  ...
  inline void gcode_M209() {
    if (MIN_AUTORETRACT <= MAX_AUTORETRACT) {
      if (parser.seen('S')) {
        ...
        for (uint8_t i = 0; i < E_STEPPERS; i++) fwretract.retracted[i] = false;
      }
    }
  }
      ...
      switch (i) {
        ...
        case E_AXIS: {
          ...
          switch (target_extruder) {
              ...
              #if AXIS_IS_TMC(E1) && E_STEPPERS > 1
                case 1: TMC_SET_CURRENT(E1); break;
              #endif
              #if AXIS_IS_TMC(E2) && E_STEPPERS > 2
                case 2: TMC_SET_CURRENT(E2); break;
              #endif
              #if AXIS_IS_TMC(E3) && E_STEPPERS > 3
                case 3: TMC_SET_CURRENT(E3); break;
              #endif
              #if AXIS_IS_TMC(E4) && E_STEPPERS > 4
...
void tool_change(const uint8_t tmp_extruder, const float fr_mm_s/*=0.0*/, bool no_move/*=false*/) {
  ...
  powerloss.Extruder = tmp_extruder;
  active_extruder = tmp_extruder;
  ...
  if (commands_in_queue == 0) {
    switch (powerloss.recovery) {
      ...
      case Rec_Recovering1:
        ...
        sprintf_P(tmp_y, PSTR("T%u"), powerloss.Extruder);
        //SERIAL_ECHOLN(tmp_y);
        enqueue_and_echo_command(tmp_y);
