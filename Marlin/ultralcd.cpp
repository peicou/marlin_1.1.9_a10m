...
void lcd_quick_feedback(const bool clear_buttons) {
  ...
  if(mixer.gradient_flag = true)
  {
    ...
    inline void _lcd_mixer_update_mix() {
      mixing_factor[NOZZLE0] = (float)((float)mixer.rate[NOZZLE0] / 100.);
      mixing_factor[NOZZLE1] = (float)((float)mixer.rate[NOZZLE1] / 100.);
    }

    inline void _lcd_mixer_toggle_mix() {
      //fgtre
      uint16_t tmp_mix = mixer.rate[NOZZLE0];
      mixer.rate[NOZZLE0] = mixer.rate[NOZZLE1];
      mixer.rate[NOZZLE1] = tmp_mix;
    ...
    void lcd_mixer_menu() {
      ...
      MENU_BACK(MSG_MAIN);
      //fgtre
      mixer.rate[NOZZLE0] = (uint16_t)(mixing_factor[NOZZLE0] * 100);
      mixer.rate[NOZZLE1] = (uint16_t)(mixing_factor[NOZZLE1] * 100);
